...
class FourierTransformUserComplex:
        ...
        while 1:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.button == 1:
                        state = FOURIER
                        x = [complex(x[0], x[1]) for x in drawing]
                        ...
                        horiz_y = 0  # int(sum(x['amp'] for x in fourier_x[1:3]))

                        num_epicycles = len(fourier_x)
